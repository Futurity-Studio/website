...
import {DividedContent, Icon, ICONS, StealthButton, THEME} from "../index";

import './landing.scss';

const Landing = ({setParentClosed}) => {
  return(
    <div className={'landing'}>
      <section>
        ...
        <div className={'buttonContainer'}>
          <div>
            <StealthButton label={'View some examples'} onClick={setParentClosed} />
          </div>
          <StealthButton
            label={'Learn How'} link={'/about'}
            icon={<Icon icon={ICONS.BULB} theme={THEME.DARK} />}
            callback={setParentClosed}/>
        </div>
      </section>
    </div>
  )
